...
| 🙂 人臉辨識 | 輸入名字採集多張樣本 → LBPH 訓練 → 人入鏡自動標出名字。資料存 `faces/`,**重開即用**;辨識門檻可調,支援中文名字 |
...
vision_server.py（唯一程式，獨佔相機）
  ...
  └─ FastAPI(背景執行緒, :8000)
       ...
       │                     obstacle,obstacle_dist,face,face_thr}
...
> **人臉辨識需要 `cv2.face`(opencv-contrib-python 提供)**。若 `import cv2; cv2.face` 失敗,請 `pip install opencv-contrib-python`。
...
- **人臉辨識**：開🙂人臉 → 輸入名字 → 對鏡頭按「＋擷取」採集 5～8 張(換角度/表情/光線) → 此人入鏡即標名字;清單可刪除、門檻越低越嚴。資料存 `faces/`,重開可用
